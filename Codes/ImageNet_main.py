...
from ImageNet_model import PreActResNet18
...
def ImageNet_train(sigma, lbd, gamma, num_classes, model, trainloader, optimizer, device):
...
def ImageNet_certify(sigma, num_classes, model, testloader, device):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='ImageNet demo')
    ...
    parser.add_argument('--resume_ckpt', default='./data/ImageNet_lg18_model/baseline.pth', type=str,
                                  help='Checkpoint path to resume')
    parser.add_argument('--ckptdir', default='./data/ImageNet_lg18_model/', type=str,
                                  help='Checkpoints save directory')
    parser.add_argument('--matdir', default='./data/ImageNet_lg18_mat/', type=str,
    ...
    parser.add_argument('--num_img', default=428, type=int,
    ...
    if args.task == 'train':
      ...
      for epoch in range(start_epoch + 1, args.epochs + 1):
          ...
          ImageNet_train(args.sigma, lbd_, args.gamma, num_classes, model, train_loader, optimizer, device)
          ...
          if epoch % 20 == 3:# and epoch >= 20:
            # Certify test
            print('===test(epoch={})==='.format(epoch))
            t1 = time.time()
            model.eval()
            ImageNet_certify(args.sigma, num_classes, model, test_loader, device)
        ...
        certify(model, device, test_dataset, None, num_classes, batch=1500,
                mode='hard', start_img=args.start_img, num_img=args.num_img, 
                sigma=0.25, matfile=(None if matdir is None else os.path.join(matdir, '{}.mat'.format(checkpoint['epoch']))), verbose=False)
