...
class Smooth(object):
  ...
  def certify(self, x: torch.tensor, n0: int, n: int, alpha: float, batch_size: int) -> (int, float):
    if self.mode == 'both':
      ...
      cAHat, _ = self.base_classifier(x.unsqueeze(0))
      cAHat = cAHat.argmax().item()
      # pdb.set_trace()
  ...
  def _sample_noise(self, x: torch.tensor, num: int, batch_size) -> np.ndarray:
    with torch.no_grad():
      ...
      for _ in range(ceil(num / batch_size)):
        ...
        # breakpoint
        # pdb.set_trace()
        noise = noise/std.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
        predictions, _ = self.base_classifier(batch + noise)
        # pdb.set_trace()
  ...
  def _count_arr(self, arr: np.ndarray, length: int) -> np.ndarray:
    ...
    for idx in arr:
      counts[idx] += 1
    # counts = np.histogram(arr, bins = length, range=(0,length))[0]
    return counts
